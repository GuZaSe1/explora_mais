...
<script>
    $(document).ready(function() {
        $('#dg_roteiros').datagrid({
            url: '/explora_mais/api/listar_roteiros_json.php',
            method: 'post',
            pagination: true,
            fitColumns: true,
            singleSelect: true,
            ...
            onExpandRow: function(index, rowRoteiro) {
                ...
                ddv.datagrid({
                    url: '/explora_mais/api/listar_itens_roteiro_json.php?roteiro_id=' + rowRoteiro.id,
                    toolbar: [{
    ...
    function abrirDialogInclusaoRoteiro() {
        ...
        $('#dlg-roteiro').load('/explora_mais/controllers/controlar_roteiro.php?form_only=1', () => $.parser.parse('#dlg-roteiro'));
    }

    function abrirDialogModificacaoRoteiro(id) {
        $('#dlg-roteiro').dialog('open').dialog('setTitle', 'Modificar Roteiro');
        $('#dlg-roteiro').load(`/explora_mais/controllers/controlar_roteiro.php?form_only=1&id=${id}`, () => $.parser.parse('#dlg-roteiro'));
    }

    function salvarRoteiro() {
        $('#fm-roteiro').form('submit', {
            url: '/explora_mais/controllers/controlar_roteiro.php',
            onSubmit: function() {
    ...
    function excluirRoteiro(id) {
        $.messager.confirm('Confirmar Exclusão', 'Tem certeza que deseja excluir este roteiro e todos os seus itens?', function(r) {
            if (r) {
                $.post('/explora_mais/controllers/excluir_roteiro.php', {
                    id: id
                }, function(result) {
                    if (result.success) {
                        $('#dg_roteiros').datagrid('reload');
    ...
    function abrirDialogInclusaoItem(roteiro_id) {
        ...
        $('#dlg-item-roteiro').load(`/explora_mais/controllers/controlar_item_roteiro.php?form_only=1&roteiro_id=${roteiro_id}`, () => $.parser.parse('#dlg-item-roteiro'));
    }

    function abrirDialogModificacaoItem(roteiro_id, id) {
        $('#dlg-item-roteiro').dialog('open').dialog('setTitle', 'Modificar Item do Roteiro');
        $('#dlg-item-roteiro').load(`/explora_mais/controllers/controlar_item_roteiro.php?form_only=1&roteiro_id=${roteiro_id}&id=${id}`, () => $.parser.parse('#dlg-item-roteiro'));
    }

    function salvarItemRoteiro() {
        $('#fm-item-roteiro').form('submit', {
            url: '/explora_mais/controllers/controlar_item_roteiro.php',
            onSubmit: function() {
                return $(this).form('validate');
            },
            success: function(result) {
    ...
    function excluirItemDoRoteiro(roteiro_id, id) {
        $.messager.confirm('Confirmar Exclusão', 'Tem certeza que deseja excluir este item do roteiro?', function(r) {
            if (r) {
                $.post('/explora_mais/controllers/excluir_item_roteiro.php', {
