...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!empty($_FILES['imagem_item']['name'])) {
        ...
        if (!in_array($ext, $ext_permitidas)) {
            ...
            $destino = $uploadDir . '/' . $nome_imagem;

            if (move_uploaded_file($arquivo['tmp_name'], $destino)) {
                $imagem_item = 'uploads/imagens/' . $nome_imagem;
            } else {
...
<form id="fm-item" method="post" enctype="multipart/form-data">
    ...
    <?php if ($imagem_item): ?>
        <img src="<?= htmlspecialchars(imagem_url($imagem_item)) ?>" style="max-width:150px;max-height:150px;">
    <?php endif; ?>
</form>
