    ...
    <div class="main-container">
        <div class="easyui-panel" title="Gerenciamento de Itens" style="padding:10px;">
            <div style="margin-bottom:10px;">
                ...
                <a href="/explora_mais/explora/admin/gerenciar_pedidos.php" class="easyui-linkbutton" data-options="iconCls:'icon-undo'">Voltar para Pedidos</a>
            </div>

            <table id="dg_itens" class="easyui-datagrid" style="width:100%; height:400px"
                data-options="url:'/explora_mais/api/listar_itens_json.php', method:'post', pagination:true, fitColumns:true, singleSelect:true, onLoadSuccess: function() { $('.easyui-linkbutton').linkbutton(); }">
                <thead>
                    <tr>
...
<script>
    function abrirDialogInclusaoItem() {
        ...
        $('#dlg-item').load('/explora_mais/controllers/controlar_item.php?form_only=1', function() {
    ...
    function abrirDialogModificacaoItem(cod_item) {
        ...
        $('#dlg-item').load('/explora_mais/controllers/controlar_item.php?form_only=1&cod_item=' + cod_item, function() {
            $.parser.parse('#dlg-item')
        })
    }
    ...
    function salvarItem() {
        ...
        $.ajax({
            url: '/explora_mais/controllers/controlar_item.php',
            type: 'post',
            data: formData,
            dataType: 'json',
    ...
    function excluirItem(cod_item) {
        $.messager.confirm('Confirmar Exclusão', 'Tem certeza que deseja excluir este item?', function(r) {
            if (r) {
                $.ajax({
                    url: '/explora_mais/controllers/excluir_item.php',
                    type: 'post',
                    data: {
                        cod_item: cod_item
                    },
