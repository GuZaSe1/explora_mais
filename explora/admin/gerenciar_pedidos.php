    ...
    <div class="main-container" style="max-width: 1200px; margin: auto;">
        <div class="easyui-panel" title="Gerenciamento de Pedidos" style="padding:10px;">
            ...
            <div style="margin-bottom:10px;">
                ...
                <a href="/explora_mais/explora/admin/gerenciar_clientes.php" class="easyui-linkbutton" data-options="iconCls:'icon-man'">Gerenciar Clientes</a>
                <a href="/explora_mais/explora/admin/gerenciar_itens.php" class="easyui-linkbutton" data-options="iconCls:'icon-tip'">Gerenciar Itens</a>
            </div>
...
<script>
    $(document).ready(function() {
        $('#dg_pedidos').datagrid({
            url: '/explora_mais/api/listar_pedidos_json.php',
            method: 'post',
            pagination: true,
            fitColumns: true,
            singleSelect: true,
            ...
            onExpandRow: function(index, rowPedido) {
                ...
                ddv.datagrid({
                    url: '/explora_mais/api/listar_itens_pedido_json.php?num_pedido=' + rowPedido.num_pedido,
    ...
    function abrirDialogInclusaoPedido() {
        ...
        $('#dlg-pedido').load('/explora_mais/controllers/controlar_pedido.php?form_only=1', () => $.parser.parse('#dlg-pedido'))
    }

    function abrirDialogModificacaoPedido(num_pedido) {
        $('#dlg-pedido').dialog('open').dialog('setTitle', 'Modificar Pedido')
        $('#dlg-pedido').load(`/explora_mais/controllers/controlar_pedido.php?form_only=1&num_pedido=${num_pedido}`, () => $.parser.parse('#dlg-pedido'))
    }

    function salvarPedido() {
        $('#fm-pedido').form('submit', {
            url: '/explora_mais/controllers/controlar_pedido.php',
            onSubmit: function() {
    ...
    function excluirPedido(num_pedido) {
        $.messager.confirm('Confirmar Exclusão', 'Tem certeza que deseja excluir este pedido e todos os seus itens?', function(r) {
            if (r) {
                $.post('/explora_mais/controllers/excluir_pedido.php', {
    ...
    function abrirDialogInclusaoItem(num_pedido) {
        ...
        $('#dlg-item-pedido').load(`/explora_mais/controllers/controlar_item_pedido.php?form_only=1&num_pedido=${num_pedido}`, () => $.parser.parse('#dlg-item-pedido'))
    }
    ...
    function abrirDialogModificacaoItem(num_pedido, num_seq_item) {
        ...
        $('#dlg-item-pedido').load(`/explora_mais/controllers/controlar_item_pedido.php?form_only=1&num_pedido=${num_pedido}&num_seq_item=${num_seq_item}`, () => $.parser.parse('#dlg-item-pedido'))
    }

    function salvarItemPedido() {
        $('#fm-item-pedido').form('submit', {
            url: '/explora_mais/controllers/controlar_item_pedido.php',
            onSubmit: function() {
                return $(this).form('validate')
            },
            success: function(result) {
    ...
    function excluirItemDoPedido(num_pedido, num_seq_item) {
        $.messager.confirm('Confirmar Exclusão', 'Tem certeza que deseja excluir este item do pedido?', function(r) {
            if (r) {
                $.post('/explora_mais/controllers/excluir_item_pedido.php', {
                    num_pedido: num_pedido,
