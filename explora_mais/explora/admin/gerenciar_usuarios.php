...
<script>
    $(document).ready(function() {
        $('#dg_usuarios').datagrid({
            url: '/explora_mais/api/listar_usuarios_json.php',
            method: 'post',
            pagination: true,
            fitColumns: true,
            singleSelect: true,
    ...
    function abrirDialogInclusao() {
        ...
        $('#dlg').load('/explora_mais/controllers/controlar_usuario.php?form_only=1', function() {
            $.parser.parse('#dlg');
        });
    }
    ...
    function abrirDialogModificacao(id) {
        ...
        $('#dlg').load('/explora_mais/controllers/controlar_usuario.php?form_only=1&id=' + id, function() {
            $.parser.parse('#dlg');
        });
    }
    ...
    function salvarUsuario() {
        ...
        $.ajax({
            url: '/explora_mais/controllers/controlar_usuario.php',
            type: 'post',
            data: form.serialize(),
            dataType: 'json',
    ...
    function excluirUsuario(id) {
        $.messager.confirm('Confirmar Exclusão', 'Tem certeza que deseja excluir este usuário?', function(r) {
            if (r) {
                $.ajax({
                    url: '/explora_mais/controllers/excluir_usuario.php',
                    type: 'post',
                    data: {
                        id: id
                    },
